...
| DB-backed session projection not rebuilt | `ExposedSessionStore.buildSession()` returns a fresh `VizSession` whose `projectionService`/`snapshot` are empty — stored events are NOT replayed into the projection on rebuild. `/events` + SSE `/stream` work (read `store.all()`); `/hierarchy` + `/threads` return 200 with empty data; `/coroutines/{id}/timeline` returns 404 even for the owner. Pre-existing persistence-layer limitation, unrelated to the CR-01/CR-02 tenancy gap (isolation still holds: cross-tenant timeline is correctly 404; owner-positive proof uses `/events` + SSE replay). Follow-up: rebuild the projection from `store.all()` in `buildSession` (or lazily on first projection read). | 03-07 | Deferred (out of scope) |
